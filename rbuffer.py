...
class ReplayBuffer_GoalEnv:
    ...
    def add(self, transition):
        ...
        # transiton is tuple of (state, action, reward, next_state, goal, done)
        self.buffer.append(transition)
        self.size +=1
    ...
    def sample(self, batch_size):
        ...
        indexes = np.random.randint(0, len(self.buffer), size=batch_size)
        states, actions, rewards, next_states, goal, dones = [], [], [], [], [],[]
        ...
        for i in indexes:
            ...
            rewards.append(np.array(self.buffer[i][2], copy=False))
            next_states.append(np.array(self.buffer[i][3], copy=False))
            goal.append(np.array(self.buffer[i][4], copy=False))
            dones.append(np.array(self.buffer[i][5], copy=False))
        
        return np.array(states), np.array(actions), np.array(rewards), np.array(next_states), np.array(goal),np.array(dones)
